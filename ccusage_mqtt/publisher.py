...
class PublisherLoop:
    ...
    def _do_header_poll(self, now_monotonic: float) -> None:
        try:
            ...
            # AnthropicAuthError is a subclass of AnthropicProbeError, so it
            # lands here too — treated as recoverable now that __main__ wraps
            # the probe in an OAuth-refresh retry. If both probe and refresh
            # fail, we still don't crash-loop; we just mark headers stale
            # until the user re-logs in (the credentials file is re-read on
            # each poll, so recovery is automatic).
